...
- external Tag Assistant and GA4 DebugView inspection,
- production verification after an authorized VPS deployment.
...
- No repository release blocker remains.
- The external GTM container and GA4 property were not available through a
  connected authorized integration, so external event receipt is not yet
  verified.
- Real file parsing and Stripe checkout were not triggered during review to
  avoid AI cost, personal-data upload and a live checkout request.
