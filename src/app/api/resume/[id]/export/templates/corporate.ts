...
function buildCorporateSectionContent(section: Section, lang: string): string {
  ...
  if (section.type === 'projects') {
    return `<div class="space-y-3">${((c as ProjectsContent).items || []).map((it: any) => `<div>
      <div class="flex items-baseline justify-between"><span class="text-sm font-bold" style="color:${NAVY}">${esc(it.name)}</span>${it.startDate ? `<span class="shrink-0 text-xs text-slate-400">${esc(it.startDate)} - ${it.endDate ? esc(it.endDate) : (lang === 'de' ? 'Heute' : 'Present')}</span>` : ''}</div>
      ${it.description ? `<div class="mt-1 text-sm text-slate-600">${md(it.description)}</div>` : ''}
      ${it.technologies?.length ? `<p class="mt-0.5 text-xs text-slate-400">${lang === 'de' ? 'Technologien' : 'Tech'}: ${esc(it.technologies.join(', '))}</p>` : ''}
      ${it.highlights?.length ? `<ul class="mt-1 list-disc pl-5">${it.highlights.map((h: string) => `<li class="text-sm text-slate-600">${md(h)}</li>`).join('')}</ul>` : ''}
    </div>`).join('')}</div>`;
  ...
  return `<div class="mx-auto max-w-[210mm] bg-white shadow-lg" style="font-family:Inter,sans-serif">
    ...
    <div class="p-8">
      ${sections.map(s => `<div class="mb-6" data-section>
        ...
        ${buildCorporateSectionContent(s, resume.language || 'de')}
      </div>`).join('')}
    </div>
  </div>`;
}
