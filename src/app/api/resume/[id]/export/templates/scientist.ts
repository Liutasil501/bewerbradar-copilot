...
function buildScientistSectionContent(section: Section, sectionIdx: number, lang: string = 'en'): string {
  ...
  if (section.type === 'work_experience') {
    return `<div class="space-y-4">${((c as WorkExperienceContent).items || []).map((it: any, idx: number) => `<div>
      <div class="flex items-baseline justify-between"><div><span class="text-xs font-bold" style="color:${ACCENT}">[${idx + 1}]</span><span class="ml-1.5 text-sm font-bold" style="color:${PRIMARY}">${esc(it.position)}</span>${it.company ? `<span class="text-sm" style="color:${MUTED}">, ${esc(it.company)}</span>` : ''}</div><span class="shrink-0 text-xs" style="color:${MUTED}">${esc(it.startDate)} - ${esc(it.endDate) || (it.current ? (lang === 'de' ? 'Heute' : 'Present') : '')}</span></div>
      ${it.description ? `<div class="mt-1 pl-6 text-sm" style="color:${BODY_TEXT}">${md(it.description)}</div>` : ''}
      ${it.technologies?.length ? `<p class="pl-6 text-xs italic" style="color:${MUTED}">${lang === 'de' ? 'Methoden/Tools' : 'Methods/Tools'}: ${esc(it.technologies.join(', '))}</p>` : ''}
  ...
  if (section.type === 'education') {
    return `<div class="space-y-3">${((c as EducationContent).items || []).map((it: any, idx: number) => `<div>
      <div class="flex items-baseline justify-between"><div><span class="text-xs font-bold" style="color:${ACCENT}">[${idx + 1}]</span><span class="ml-1.5 text-sm font-bold" style="color:${PRIMARY}">${esc(degreeField(it.degree, it.field))}</span>${it.institution ? `<span class="text-sm" style="color:${MUTED}">, ${esc(it.institution)}</span>` : ''}</div><span class="shrink-0 text-xs" style="color:${MUTED}">${esc(it.startDate)} - ${esc(it.endDate) || (lang === 'de' ? 'Heute' : 'Present')}</span></div>
  ...
  if (section.type === 'projects') {
    return `<div class="space-y-3">${((c as ProjectsContent).items || []).map((it: any, idx: number) => `<div>
      <div class="flex items-baseline justify-between"><div><span class="text-xs font-bold" style="color:${ACCENT}">[${idx + 1}]</span><span class="ml-1.5 text-sm font-bold" style="color:${PRIMARY}">${esc(it.name)}</span></div>${it.startDate ? `<span class="shrink-0 text-xs" style="color:${MUTED}">${esc(it.startDate)} - ${it.endDate ? esc(it.endDate) : (lang === 'de' ? 'Heute' : 'Present')}</span>` : ''}</div>
      ${it.description ? `<div class="mt-1 pl-6 text-sm" style="color:${BODY_TEXT}">${md(it.description)}</div>` : ''}
      ${it.technologies?.length ? `<p class="pl-6 text-xs italic" style="color:${MUTED}">${lang === 'de' ? 'Methoden/Tools' : 'Methods/Tools'}: ${esc(it.technologies.join(', '))}</p>` : ''}
      ${it.highlights?.length ? `<ul class="mt-1 pl-6 space-y-0.5">${it.highlights.filter(Boolean).map((h: string) => `<li class="flex items-start gap-2 text-sm" style="color:${BODY_TEXT}"><span class="mt-1.5 shrink-0 text-xs" style="color:${ACCENT}">-</span>${md(h)}</li>`).join('')}</ul>` : ''}
    </div>`).join('')}</div>`;
